...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=208, patch_size=16, in_c=3, embed_dim=768, norm_layer=None):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        # [batch_size, num_patches + 1, total_embed_dim]       batch_size,169+1,768
        B, N, C = x.shape
...
class ChannelAttention(nn.Module):
    def __init__(self,
        ...
        ## channel attention
        self.channel_num_heads = 13  # num_heads
        N = 13 * 13  # dim
    ...
    def forward(self, x):
        ...
        assert N == 85 or N == 170 or N == 43, 'N must be 14 * 14 * mask_ratio + 1, mask_ratio in [0, 0.5, 0.75]'
        cls_tokens_x = x[:, :1, :]  # [B, 1, N]
        x = x[:, 1:, :].transpose(-2, -1)  # [B, N+1, C]->[B, C, N]
        if N == 85:
            x = x.repeat_interleave(2, 2)  # [B,C,98]->[B,C,169]
            x = x.repeat(1, 1, 2)[:, :, :169]
        elif N == 43:
            x = x.repeat_interleave(4, 2)  # [B,C,49]->[B,C,169]
            x = x.repeat(1, 1, 2)[:, :, :169]
        # x = x.repeat_interleave(math.ceil(169 / (N - 1)), 2)[:, :, :169]  # match N to 169

        qkv_channel = self.channel_qkv(x).reshape(B, C, 3, self.channel_num_heads,
                                                  169 // self.channel_num_heads).permute(2, 0, 3, 1, 4)  # N*2=169
        ...
        x = (attn_channel @ v_channel).transpose(1, 2).reshape(B, C, 169)  # [5,14,32,14]->[5,32,14,14]->[5,32,169]
        x = self.channel_proj(x)
        x = self.channel_proj_drop(x)
        if N == 85:
            x = (x[:, :, 0:-2:2] + x[:, :, 1::2]) / 2  # [5,32,169]->[5,32,98]
        elif N == 43:
            x = (x[:, :, 0:-2:4] + x[:, :, 1::4] + x[:, :, 2::4] + x[:, :, 3::4]) / 4  # [5,32,169]->[5,32,49]
...
class DenseFormer(nn.Module):
    def __init__(self, img_size=208, patch_size=16, in_c=3,
...
if __name__ == '__main__':
    input = torch.randn(64, 3, 208, 208)
